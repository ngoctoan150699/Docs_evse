...
- **`POST /api/mobile/me/vehicles/{id}/autocharge`**: Kích hoạt tính năng **Autocharge** (gán mã định danh phần cứng `EVCCID` / MAC của xe vào xe điện để cắm súng là tự sạc). Tự động chuẩn hóa MAC, kiểm tra độ dài 6-32 ký tự và ràng buộc độc bản duy nhất trong Tenant.
- **`DELETE /api/mobile/me/vehicles/{id}/autocharge`**: Hủy kích hoạt tính năng Autocharge cho xe.
- **`GET /api/mobile/me/detected-evcc`**: Lấy mã `EVCCID` vừa được trạm sạc phát hiện khi cắm cáp CCS2 gần nhất (Zero-Touch Onboarding hỗ trợ tự động điền form).
...
- **`POST /api/csms/customers/{id}/vehicles/{vehicleId}/autocharge`**: Admin bật/tắt Autocharge cho xe của khách hàng (yêu cầu quyền quản lý trạm `CanWriteStation`, tự động ghi vết an ninh `audit_logs`).
- **`GET /api/csms/settings` & `PUT /api/csms/settings`**: Xem và bật/tắt tham số Kill-Switch toàn hệ thống `autocharge_whitelist_enabled` (chỉ SuperAdmin / TenantAdmin).
